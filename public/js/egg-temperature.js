...
function deleteRecord(targetID) {
    ...
    .then(response => {
        ...
        return response.text(); // Read raw response before parsing
    })
    .then(text => {
    ...
    .catch(error => {
        console.error("Error:", error);
        createPushNotification("danger", "Delete Failed", "An error occurred while deleting. Refreshing...");
...
function editRecord(targetID) {
    ...
    .catch(error => {
        console.error("Error:", error);
        createPushNotification("danger", "Edit Failed", "An error occurred while processing. Please try again.");
        setTimeout(() => {
            location.reload(); // Refresh after 3s
        }, 3000);
    });

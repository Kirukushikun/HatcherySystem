    ...
    <script>
        ...
        document.addEventListener("click", function (event) {
            if (event.target.closest(".AccessBtn")) {
                ...
                let role = button.getAttribute("data-role"); // Assuming role is set in the button
                let action = button.getAttribute("data-action");

                Swal.fire({
                    title: "Are You Sure?",
                    text: `That You Want To ` + (action === `revoke` ? `Revoke` : `Grant`) + ` Access to ${fullname}?`,
                    icon: "warning",
                    ...
                    if (result.isConfirmed) {
                        fetch(`/users/grant-access/${id}/${role}`, {
                            method: "GET",
                            headers: { "X-Requested-With": "XMLHttpRequest" },

                        })
                        .then(response => response.json()) // Assuming Laravel returns JSON
                        .then(data => {
                            Swal.fire("Access " + (action === "revoke" ? "Revoked" : "Granted") + "!", `${fullname} has ` + (action === "revoke" ? "no" : "") + ` access to the hatchery system.`, "success");
                        })
                        .catch(error => {
